...
export const uswdsComponentDefinitions = {
  ...
  // ── CMS Meta Components ───────────────────────────────────────────────────
  // Higher-level domain components for CMS/VA/Medicare UI. Each maps to a
  // specific gov-service context rather than a generic web UI primitive.
  //
  // IMPORTANT: Zod schemas are defined here (validation authority) but there
  // are NO static React implementations in components.tsx for these entries.
  // Implementations are web components served at runtime from the external CDN
  // (packages/mock-cdn). The playground registry builder (lib/render/registry.ts)
  // fetches CDN catalog entries and creates adapters via createWebComponentAdapter.

  PlanInformation: {
    props: z.object({
      planType: z.string().nullish().describe("Plan category label, e.g. 'Original Medicare'"),
      planName: z.string().nullish().describe("Full plan name; newline characters render as line breaks"),
      partACoverageDate: z.string().nullish().describe("Part A coverage start date, e.g. '8/12/2019'"),
      partBCoverageDate: z.string().nullish().describe("Part B coverage start date, e.g. '8/12/2019'"),
      detailsHref: z.string().nullish().describe("URL for the 'Coverage details' link"),
      detailsLabel: z.string().nullish().describe("Link text; defaults to 'Coverage details'"),
    }),
    description:
      "Medicare plan summary panel. Shows plan type header, bold plan name, Part A/B coverage start dates, and a details link. Prefer over Card+Text composition for Medicare plan display.",
    example: {
  ...
  AppointmentCard: {
    props: z.object({
      providerName: z.string().nullish().describe("e.g. 'Dr. Sarah Chen'"),
      facility: z.string().nullish().describe("e.g. 'VA Portland Medical Center'"),
      appointmentDate: z.string().nullish().describe("Display date string, e.g. 'Mon, May 5, 2026'"),
      appointmentTime: z.string().nullish().describe("Display time string, e.g. '10:30 AM'"),
      appointmentType: z.enum(["in_person", "telehealth"]).nullish(),
      department: z.string().nullish().describe("e.g. 'Primary Care'"),
      status: z.enum(["scheduled", "completed", "cancelled"]).nullish(),
      notes: z.string().nullish(),
    }),
    description:
      "VA appointment summary card. Shows provider, facility, date/time, type, and status. Prefer over Card+Text composition for VA appointment display.",
    example: {
      providerName: "Dr. Sarah Chen",
      facility: "VA Portland Medical Center",
      appointmentDate: "Mon, May 5, 2026",
  ...
  ClaimStatusTimeline: {
    props: z.object({
      ...
      claimType: z.string().nullish().describe("e.g. 'Disability Compensation'"),
      filedDate: z.string().nullish().describe("Display date string, e.g. 'March 1, 2026'"),
      currentStepNumber: z
        .number()
        .int()
        .min(1)
        .max(5)
        .nullish()
        .describe("Active step 1–5: 1=Received, 2=Review, 3=Evidence, 4=Rating, 5=Decision"),
      currentStepLabel: z.string().nullish().describe("Human-readable label for the current step"),
      estimatedDecisionDate: z.string().nullish().describe("e.g. 'June 2026'"),
      lastUpdated: z.string().nullish(),
    }),
    description:
      "VA claim status timeline. Shows 5-step progress (Received → Review → Evidence → Rating → Decision) with current step highlighted. Prefer over ProcessList composition for VA claim status display.",
    example: {
      claimNumber: "123456789",
      claimType: "Disability Compensation",
      filedDate: "March 1, 2026",
      currentStepNumber: 3,
      currentStepLabel: "Evidence Gathering",
      estimatedDecisionDate: "June 2026",
    },
  },

  BenefitSummary: {
    props: z.object({
      program: z.string().nullish().describe("e.g. 'Post-9/11 GI Bill (Chapter 33)'"),
      totalEntitlement: z.string().nullish().describe("e.g. '36 months'"),
      used: z.string().nullish().describe("e.g. '24 months 15 days'"),
      remaining: z.string().nullish().describe("e.g. '11 months 15 days'"),
      expirationDate: z.string().nullish().describe("Benefit expiration date"),
      school: z.string().nullish().describe("Currently enrolled school name"),
      enrollmentStatus: z.string().nullish().describe("e.g. 'Full-time'"),
    }),
    description:
      "GI Bill / benefit entitlement summary panel. Shows program, total/used/remaining entitlement, expiration, and current enrollment. Prefer over Card+Text composition for GI Bill or benefit display.",
    example: {
  ...
  FacilityCard: {
    props: z.object({
      ...
      facilityType: z.enum(["health", "benefits", "cemetery", "vet_center"]).nullish(),
      address: z.string().nullish(),
      phone: z.string().nullish(),
      distance: z.string().nullish().describe("e.g. '3.2 miles'"),
      hours: z.string().nullish().describe("e.g. 'Mon–Fri 8:00 AM – 4:30 PM'"),
      waitTime: z.string().nullish().describe("Average wait time, e.g. '12 days average'"),
    }),
    description:
      "VA facility locator card. Shows facility name, type badge, address, phone, distance, and hours. Prefer over Card+Text composition for VA facility display.",
    example: {
      facilityName: "VA Portland Medical Center",
      facilityType: "health",
